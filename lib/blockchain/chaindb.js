...
class ChainDB {
  ...
  async scan(start, filter, iter) {
    ...
    while (entry) {
      console.log('scan:'+entry.hash+",height:"+entry.height);

      // //xd added for test start
      // console.log(typeof(entry.hash))
      // console.log(typeof(entry.height))
      // console.log(entry.height)
      // if(entry.height==0)
      // {
      //   entry.hash='042260bfd317b15351ba54c84515c80d81284202746ccbf13c461e9406000000';
      //   // 00000006941e463cf1cb6c74024228810dc81545c854ba5153b117d3bf602204
      // }
      //
      // if(entry.height==1)
      // {
      //   entry.hash='21c0af0665fed05f890021b9b9d29639ca7b8fac3791c8281068e8e7d802000000';
      //   // 21c0af0665fed05f890021b9b9d29639ca7b8fac3791c8281068e8e7d802000000
      // }
      // // xd added for test end
      ...
      if (!block) {
        if (!this.options.spv && !this.options.prune)   //xd added
          throw new Error('Block not found.5:'+entry.hash);
  ...
  async reset(block) {
    ...
    if (!entry)
      throw new Error('Block not found.6');
  ...
  async removeBlock(entry) {
    ...
    if (!block)
      throw new Error('Block not found.7');
  ...
  async connectBlock(entry, block, view) {
    ...
    // xd modified start
    // // Genesis block's coinbase is unspendable.   //xd added
    // if (entry.isGenesis())
    //   return undefined;
    // xd modified end
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      if (i > 0) {
        for (const {prevout} of tx.inputs)
          ...
          // xd modified start 规避方案 
          if(   hash.toString('hex')!='37fac5029eae857f9983ded5cbff1eec6219219e74cfcd8d81cbc50a00000000'
            && (prevout.hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && prevout.index != 279)
            && (prevout.hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && prevout.index != 138)
            && (prevout.hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && prevout.index != 20)){
            // console.log(prevout)
  ...
  indexTX(tx, view, entry, index) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        // xd added for 4642 start
        if((hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && index != 279)
          && (hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && index != 138)
          && (hash!='7b3e8f0a5371086cf000d11b5e6bd1f8779980af855623b6eb28c4d63d3b12a4' && index != 20)) {
          assert(coin);
        } else {
          continue;
        }
