...
const Block = require('../primitives/block');  //xd added
...
class Chain extends AsyncEmitter {
  ...
  async verifygenesis(block, prev, flags) {
    assert(typeof flags === 'number');
  ...
  async updateInputs(block, prev) {
    ...
    // xd added start
    // xd added for genesis start
    if(block.prevBlock === this.network.genesis.hash) {
      const genesis = this.network.genesisBlock;
      const genesisBlock = Block.fromRaw(genesis, 'hex');
      const genesisHeight = 0;

      for (let i = 0; i < genesisBlock.txs.length; i++) {
        const tx = genesisBlock.txs[i];
        view.addTX(tx, genesisHeight);
      }

    }
    // xd added for genesis end
    ...
    if(height != 4642) { // xd added for 4642
      for (let i = 1; i < block.txs.length; i++) {
        const tx = block.txs[i];

        assert(await view.spendInputs(this.db, tx),
          'BUG: Spent inputs in historical data!'+height+':'+tx.toString('hex'));
  ...
  async verifyInputs(block, prev, state) {
    ...
    if(  height==5151
      ...
      || height==36150
      || height==37906
      || height==38128)  //xd added 高度5151不校验
    {
      console.log(reward);
      return view
    }
  ...
  async disconnect(entry) {
    ...
    if (!block) {
      if (!this.options.spv)
        throw new Error('Block not found.2');
  ...
  async reconnect(entry) {
    ...
    if (!block) {
      if (!this.options.spv)
        throw new Error('Block not found.3');
      block = entry.toHeaders();
    }
  ...
  async setBestChain(entry, block, prev, flags) {
    ...
    try {
      ...
      console.log("******setBestChain error***********");
      console.log(err)
      if (err.type === 'VerifyError') {
        if (!err.malleated)
          this.setInvalid(entry.hash);
  ...
  // xd added start  
  async saveAlternategenesis(entry, block, prev, flags) {
    try {
      // Do as much verification
      // as we can before saving.
      await this.verifygenesis(block, prev, flags);
    } catch (err) {
      if (err.type === 'VerifyError') {
        if (!err.malleated)
          this.setInvalid(entry.hash);
    ...
    this.logger.warning('saveAlternategenesis Heads up: Competing chain at height %d:'
      + ' tip-height=%d competitor-height=%d'
      + ' tip-hash=%s competitor-hash=%s'
      + ' tip-chainwork=%s competitor-chainwork=%s'
      + ' chainwork-diff=%s',
  ...
  async _reset(block, silent) {
    ...
    this.height = tip.height;
    this.synced = false;
    console.log(" this.synced = false 1=============================================================================== start")
    console.log(" this.synced = false 1=============================================================================== end")
  ...
  async _replay(block, silent) {
    ...
    if (!entry)
      throw new Error('Block not found.4');
   ...
   */
  // xd added start
  async addGenesis(block, flags, id) {
    // console.trace();
    const hash = block.hash('hex');
    const unlock = await this.locker.lock(hash);
    try {
      return await this._addGenesis(block, flags, id);
    } finally {
      unlock();
    }
  }
  ...
  // xd added start
  async _addGenesis(block, flags, id) {

    // console.log("========================async _add(block, flags, id)================================start");
    // console.trace();
    // console.log("========================async _add(block, flags, id)================================end");
    ...
    //   const genesis = this.network.genesisBlock;
    //   const genesisblock = Block.fromRaw(genesis, 'hex');
    //   const genesisprev = await this.getEntry(genesisblock.prevBlock);
    //   const entry = await this.connectgenesis(genesisprev, genesisblock, flags);
    // }
    // //xd added end

    // Connect the block.
    const entry = await this.connectgenesis(prev, block, flags);
  ...
  async connectgenesis(prev, block, flags) {
    ...
    if (entry.chainwork.lte(this.tip.chainwork)) {
      ...
      // await this.saveAlternategenesis(entry, block, prev, flags);
      await this.saveAlternategenesis(entry, block, prev, flags);
    } else {
  ...
  maybeSync() {
    ...
    this.synced = true;
    console.log(" this.synced========================================================================================ture 1")
    console.log(" this.synced========================================================================================ture 2")
    console.log(" this.synced========================================================================================ture 3")
    console.log(" this.synced========================================================================================ture 4")
    this.emit('full');
  }
  ...
  async getTarget(time, prev) {
    ...
    if ((prev.height + 1) % pow.retargetInterval !== 0) {
      ...
      //xd added start
      let targetReset= true;
      // if(this.network === Network.main && prev.height +1 >= 30860 ) {

      if(prev.height +1 >= 30860 ) {
        targetReset = false;
      }
      ...
      // if (pow.targetReset) { // xd modified
      if (targetReset) {
