...
exports.encode = function encode(num, exp) {
  // assert(Number.isSafeInteger(num), 'Invalid integer value.'); // xd added 数值过大，需要适配
...
exports.decode = function decode(str, exp) {
  ...
  // assert(hi.length <= 16 - exp,                  // xd added  数值过大，需要适配
  ...
  // assert(hi < maxHi || (hi === maxHi && lo <= maxLo),   // xd added 数值过大，需要适配
  //   'Fixed number string exceeds 2^53-1.');
