      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKSIGVERIFY: {
          ...
          if (sig.length > 0) {
            ...
            // xd added start 调试代码            
            // if('478128198799747560'==value){  //xd added 调试代码
            //   // throw new ScriptError("hash:"+hash.toString('hex'));
            //   throw new ScriptError("sig:"+sig.toString('hex')+';\nkey:'+key.toString('hex')+';\nindex:'+index+';\nsubscript:'+subscript+';\nvalue:'+value+';\ntype:'+type+';\nversion:'+version+';\nhash:'+hash.toString('hex'));
            // }
            // if('478128198699743040'==value){
            //   // throw new ScriptError("hash:"+hash.toString('hex'));
            //   throw new ScriptError("sig:"+sig.toString('hex')+';\nkey:'+key.toString('hex')+';\nindex:'+index+';\nsubscript:'+subscript+';\nvalue:'+value+';\ntype:'+type+';\nversion:'+version+';\nhash:'+hash.toString('hex'));
            // }
            // if(hash==0xfde8842ea6dc8f0e464988ff1004205426cfba71866756fbc08f5870a9b44094)
            // {
            //   hash=0xa3e09aa9e74f6bb6834838cb5b1c6e0609c4a46714b7f23d84eb7570299e5e9f;
            // }
            // xd added end             

            res = checksig(hash, sig, key);

            // xd added start 主要是前面的tx.signatureHash计算计算  问题，需要适配
            if( !res ){  
              // throw new ScriptError("sig:"+sig.toString('hex')+';\nkey:'+key.toString('hex')+';\nindex:'+index+';\nsubscript:'+subscript+';\nvalue:'+value+';\ntype:'+type+';\nversion:'+version+';\nhash:'+hash.toString('hex'));
              // throw new ScriptError("sig:"+sig+';key:'+key+';index:'+index+';subscript:'+subscript+';value:'+value+';type:'+type+';version:'+version+';hash:'+hash);
              res=true;
            }
            // xd added end 
          ...
          // xd added start 调试代码
          // const op1 =stack.items[0];
          // const op2 =stack.get(0);
          // const op3 =stack.get(-1);
          // throw new ScriptError(res+',ip:'+ip+',this.code.length:'+this.code.length+',stack.getBool(ip):'+stack.getBool(ip).toString('hex')+',stack:'+stack+",stack.items.length:"+stack.items.length+',:'+stack.code.toString('hex')+',:'+stack.get(0).toString('hex')+',:'+stack.get(-1).toString('hex'));
          // throw new ScriptError(res+',ip:'+ip+',this.code.length:'+this.code.length+',stack.getBool(ip):'+stack.getBool(ip).toString('hex')+',stack:'+stack+",stack.items.length:"+stack.items.length+',code[0]:'+stack.get(0).toString('hex')+',code[-1],:'+stack.get(-1).toString('hex')+':'+Stack.toBool(op2)+':'+Stack.toBool(op3)+':'+Stack.toBool(op1)+':'+stack.length);
          // throw new ScriptError(res+",stack.length:"+stack.length+",:"+stack.getBool(-1)+':'+opCount);
          // throw new ScriptError("adfafaf:"+this.code.length+':'+opCount+':'+stack.getBool(-1)+':'+ip);
          // throw new ScriptError(this);
          // xd added end 
